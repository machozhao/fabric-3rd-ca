...
PEER_NAMES="peer0 peer1 peer2"
PEER_ADMIN_USERS="Admin"
PEER_USERS="User1 User2"
...
do
	...
	for ORG_DOMAIN_NAME in $ORG_DOMAIN_NAMES
		...
		do
			...
			cp "${CA_CERT_FILE}" "${PEER_ECERT_OUTPUT_DIR}/msp/tlscacerts"
			...
			cp "${CA_CERT_FILE}" "${CRYPTO_CONFIG_DIR}/${TOP_NODE}/${ORG_DOMAIN_NAME}/msp/tlscacerts/"
		...
		do
			...
			cp "${CA_CERT_FILE}" "${CRYPTO_CONFIG_DIR}/${TOP_NODE}/${ORG_DOMAIN_NAME}/users/${PEER_ADMIN_USER}@${ORG_DOMAIN_NAME}/msp/tlscacerts"
		...
		do
			...
			cp "${CA_CERT_FILE}" "${CRYPTO_CONFIG_DIR}/${TOP_NODE}/${ORG_DOMAIN_NAME}/users/${PEER_USER}@${ORG_DOMAIN_NAME}/msp/tlscacerts"
